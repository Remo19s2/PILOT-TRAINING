...
class NegotiationService:
    def draft(self, db: Session, user: User, quotation_id: UUID, dimensions: list[str]) -> dict:
        ...
        if not negotiation:
            negotiation = Negotiation(id=uuid4(), rfq_id=quotation.rfq_id, quotation_id=quotation.id, supplier_id=quotation.supplier_id, status="DRAFT")
            db.add(negotiation)
            db.flush()
        ...
        message = NegotiationMessage(id=uuid4(), negotiation_id=negotiation.id, sender_type="AI", message_type="DRAFT", content=json.dumps(content))
        db.add(message)
    ...
    def authorize(self, db: Session, user: User, negotiation_id: UUID, content: str) -> dict:
        negotiation = db.get(Negotiation, negotiation_id)
        if not negotiation:
            raise ValueError("Negotiation not found")
        message = NegotiationMessage(id=uuid4(), negotiation_id=negotiation.id, sender_type="PROCUREMENT_MANAGER", message_type="AUTHORIZED_OUTBOUND", content=content, authorized_by=user.id)
